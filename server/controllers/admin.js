...
//     const products = await product
//       .find() // No filtering based on searchTerm
//       .skip((page - 1) * parseInt(limit)) // Pagination
//       .limit(parseInt(limit)) // Limit results per page
//       .sort("brand name"); // Sort by brand and name
...
//       image: product.image,
//       name: product.name,
//       desc: `${(product.ratingScore / (product.ratings?.length || 1)).toFixed(
//         1
//       )} stars, ${product.color}`,
//       size: product.sizeQuantity
//         .map((size) => `${size.size} (${size.quantity} unit)`)
...
const getAllProducts = asyncErrorHandler(async (req, res) => {
  try {
    ...
    const searchQuery = typeof searchTerm === "string" ? searchTerm : "";
